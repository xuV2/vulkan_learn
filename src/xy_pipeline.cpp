...
namespace xy{
    ...
    XyPipeline::~XyPipeline(){
        vkDestroyShaderModule(xyDevice.device(),vertShaderModule,nullptr);
        vkDestroyShaderModule(xyDevice.device(),fragShaderModule,nullptr);
        vkDestroyPipeline(xyDevice.device(),graphicsPipeline,nullptr);
    }
    ...
    std::vector<char> XyPipeline::readFile(const std::string filePath){
        ...
        file.seekg(0); // 璁剧疆鏂囦欢娴佺殑璇绘寚閽堜綅缃�俿eekg 鏄? "seek get" 
    ...
    void XyPipeline::createGraphicsPipeline(const std::string &vertFilePath,const std::string &fragFilePath,PipelineConfigInfo configInfo){
        ...
        createShaderMoule(vertCode,&vertShaderModule);
        ...
        if(vkCreateGraphicsPipelines(xyDevice.device(),VK_NULL_HANDLE,1,&pipelineInfo,nullptr,&graphicsPipeline) !=VK_SUCCESS){
    ...
    PipelineConfigInfo XyPipeline::defaultPipelineConfigInfo(uint32_t width, uint32_t height){
            ...
            configInfo.inputAssemblyInfo.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;  // 图元类型，点列表
            configInfo.inputAssemblyInfo.primitiveRestartEnable = VK_FALSE;  // 禁用图元重启
